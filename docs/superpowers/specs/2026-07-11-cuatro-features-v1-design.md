...
- **KikiCore:** nuevo protocolo `StreamTranscribing` (start/stop + callback de parciales) junto al `Transcribing` batch. `DictationController` gana modo live: `recording` con parciales fluyendo → al stop el resultado es lo acumulado; **no hay fase `processing` en live**. [Ajustado en implementación: hay una fase `.processing` breve durante el pass final; la burbuja permanece con spinner y se limpia tras insertar.] Los snippets SÍ corren en modo live (expansiones determinísticas definidas por el usuario, no LLM — intencional).
